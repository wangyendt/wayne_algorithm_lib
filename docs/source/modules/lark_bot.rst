...
本模块实现了与飞书平台进行交互的功能，主要用于发送文本、图片、文件、音频以及可交互消息等消息。该模块包含以下主要部分：

1. TextContent 类：提供各种文本格式化工具，用于生成消息中所需的特殊格式（例如加粗、斜体、下划线、@某人及链接等）。
2. PostContent 类：用于构造 post 富文本内容，支持 Markdown 分块与表格降级渲染。
3. CardContentV2 类：用于构造 schema 2.0 interactive 卡片（支持 markdown/hr/img 组件）。
4. LarkBot 类：核心类，管理与飞书平台的交互，支持发送不同类型的消息，并提供用户和群组信息查询等功能。
5. LarkBotListener 类：提供飞书消息监听和处理功能。
...
- ``add_markdown(md, table_as="code_block", max_chunk_bytes=8000, mono_max_col_width=40)``
  直接添加 Markdown 文本。内部会自动按字节分块；当检测到 Markdown 表格时，默认降级为 ``code_block`` 等宽表格渲染（也可通过 ``table_as="md"`` 保持原始 Markdown）。

用户可以根据实际需求将这些内容方法组合使用，以构造丰富且格式化的消息。

示例：post 富文本中的 Markdown 表格安全发送

.. code-block:: python

   from pywayne.lark_bot import PostContent

   post = PostContent(title="发布计划")
   md = """
   ## 里程碑
   ...
   | P1  | 进行中 | Bob |
   """
   post.add_markdown(md, table_as="code_block")


CardContentV2 类
----------------

CardContentV2 是 schema 2.0 卡片构造器，适合优先走 interactive 卡片发送 Markdown 内容。主要方法包括：

- ``__init__(title="", template="blue")``
  初始化最小可用 schema 2.0 卡片结构。

- ``add_markdown(md, max_chunk_bytes=18000)``
  添加 markdown 组件，自动按字节分块，避免单卡内容过大。

- ``add_hr()``
  添加分割线。

- ``add_image(img_key, size="large", preview=True)``
  添加图片组件。

- ``get_card()``
  获取完整卡片 JSON，可直接交给 ``send_interactive_to_chat`` 发送。

示例：构造并发送 schema 2.0 markdown 卡片
...
.. code-block:: python
   ...
   card = CardContentV2(title="日报")
   card.add_markdown("# 今日进展\n\n- 完成接口联调\n- 修复 2 个线上问题")
   bot.send_interactive_to_chat(chat_id="oc_xxx", interactive=card.get_card())
...
- **一键发送 Markdown（推荐）**：
  - ``send_markdown_to_chat(chat_id, md_text, title="", prefer="card_v2", table_fallback="code_block", max_message_bytes=None)``：默认优先以 ``card_v2`` 路由发送（schema 2.0 interactive 卡片）；可切换到 ``post`` 路由。该方法会自动按字节分包发送多条消息，避免超限；在 ``prefer="post"`` 时可通过 ``table_fallback`` 控制表格降级策略。
...
.. code-block:: python
   ...
   # 推荐：直接发送 Markdown（默认走 schema 2.0 card）
   md = """
   # 发布公告
