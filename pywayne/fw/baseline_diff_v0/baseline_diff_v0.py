    ...
    def force_flag_detect_I_II(self, rawdata, baseline, fbase, force_flag,
        ...
        if force_flag == 0:
            ...
            if 1 or not temperature_err_flag:  # 常温逻辑
                ...
                elif sig[last_maxch] < LEAVE_COEFF / 2 * self.sig_peak[last_maxch]:
                    leave_type = -0.2  # 常温比值阈值大通道
                    self.state_dict['leave_type'] = leave_type
                elif sig[last_maxch] < LEAVE_COEFF * self.sig_peak[last_maxch] and \
                        sig[second_ch] < LEAVE_COEFF / 2 * self.sig_peak[second_ch]:
                    leave_type = -0.3  # 常温比值阈值小通道
                    self.state_dict['leave_type'] = leave_type
...
if __name__ == '__main__':
    from .test_baseline_diff import test_baseline
    test_baseline()
