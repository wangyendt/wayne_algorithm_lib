...
from .force_flag_detect_son_functions import calc_slpcoef_rebound, calc_forceflagzero_params, \
    pop_state_dict, update_sigpeak_lastmaxch, initialize_sig_peak
...
class BaseLine:
    def __init__(self, rawdata):
        ...
        self.sig_peak_diff = np.ones((2,)) * (-1000)
    ...
    def _plot_slow_rule(self, start_frame, i, only_plot_ch56):
    ...
    def __update_sigpeak_diff(self, sig, sig_peak):
        for ch in range(2):  # 更新 sig_peak
            sig_peak[ch] = sig[ch] if sig[ch] > sig_peak[ch] else sig_peak[ch]

    def deepcopy_state(self, bsl):
        self.state_dict = deepcopy(bsl.state_dict)
        self.stable_cnt = deepcopy(bsl.stable_cnt)
        self.stable_state_sw = deepcopy(bsl.stable_state_sw)
        # self.leave_type = deepcopy(bsl.leave_type)
    ...
    def force_flag_detect_I_II(self, rawdata, baseline, fbase, force_flag,
        ...
        # force_flag == 1
            ...
            # 常温逻辑
            if 1 or not temperature_err_flag:  # 常温逻辑
                diff_sig = np.zeros((2,))
                k = min(0 + touch_cnt * 0.01, 1.3)
                if last_maxch in [0, 1]:
                    diff_sig[0] = sig[0] - k * sig[2]
                    diff_sig[1] = sig[1] - k * sig[3]
                elif last_maxch in [2, 3]:
                    diff_sig[0] = -k * sig[0] + sig[2]
                    diff_sig[1] = -k * sig[1] + sig[3]
                else:
                    diff_sig[0] = sig[4]
                    diff_sig[1] = sig[5]
                self.__update_sigpeak_diff(diff_sig, self.sig_peak_diff)
                last_maxch_01 = (last_maxch > second_ch) * 1
                if diff_sig[last_maxch_01] < LEAVE_TH:
                    leave_type = -0.1  # 常温力阈值
                    self.state_dict['leave_type'] = leave_type
                elif diff_sig[last_maxch_01] < LEAVE_COEFF * self.sig_peak_diff[last_maxch_01]:
                    leave_type = -0.2  # 常温比值阈值大通道
                    self.state_dict['leave_type'] = leave_type
                leave_type = self.state_dict['leave_type']
                if (rawdata[-1, last_maxch] - rawdata[-3, last_maxch]) > 0 and \
                        (leave_type == -0.1 or leave_type == -0.2 or leave_type == -0.3):
            ...
            if leave_type > 0:  # 离手时处理
                ...
                self.state_dict['leave_type'] = 0
                self.state_dict['last_peak'] = np.max(self.sig_peak)
                self.state_dict['last_maxch'] = [-1, -1]
                initialize_sig_peak(self.sig_peak)
                initialize_sig_peak(self.sig_peak_diff)
                # 离手1帧拉回基线
                if temperature_err_flag:
                    valley_pos = 5 - np.argmin(rawdata[- 5:, last_maxch])
                    for ch in range(0, 6):
                        if ch in [4, 5]:
    ...
    def run_example(self, start_frame=100, plot_out_flag=False, plot_slow_release_flag=False, suptitle='',
        ...
        :param forceflag_fw      分析固件forceflag
        :return:
        ...
        for i in range(start_frame, np.shape(self.rawdata)[0]):
            self.forceflag_sw[i], self.leave_type[i] = \
                self.force_flag_detect_I_II(self.rawdata[(i - 3 * ORDER16 - 2):i + 1],
                                            self.baseline_sw[i - 1],
                                            self.fbase_sw[i - 1],
                                            self.forceflag_sw[i - 1],
                                            temperature_err_flag=1, sig_err_flag=0)
            # 统计离手原因,并return
            if self.forceflag_sw[i - 1] == 1 and self.forceflag_sw[i] == 0 and statistic_flag:
                assert (forceflag_true is not None)
                assert (forceflag_true.shape == self.forceflag_sw.shape)
                true_end_id = np.where(np.diff(forceflag_true) == -1)[0]
                assert (true_end_id.shape[0] == 1)
        ...
        if (not statistic_flag) and (plot_slow_release_flag or plot_out_flag):
            ...
            for ch in range(CH_NUMS):
                ...
                plt.plot(self.rawdata[:, ch], '--')
                plt.plot(self.baseline_sw[:, ch])
                plt.plot(self.forceflag_sw * (self.rawdata[:, ch].max() - self.rawdata[:, ch].min())
                         + self.rawdata[:, ch].min())
                plt.legend(('rawdata', 'baseline', 'forceflag'))
                if forceflag_true is not None:
                    assert (forceflag_true.shape == self.forceflag_sw.shape)
            ...
            # 图2. 画差分图
            plt.figure()
            plt.suptitle(suptitle + '+ forcesig差分')
            forcesig = self.rawdata - self.baseline_sw
            forcesig1 = np.copy(forcesig)
            forcesig1[:, 0] = forcesig[:, 0] - forcesig[:, 2]
            forcesig1[:, 1] = forcesig[:, 1] - forcesig[:, 3]
            forcesig1[:, 2] = forcesig[:, 2] - forcesig[:, 0]
            forcesig1[:, 3] = forcesig[:, 3] - forcesig[:, 1]
            for ch in range(CH_NUMS):
                plt.subplot(3, 2, ch + 1)
                if ch == 0:
                    plt.title('ch1-ch3')
                    ...
                    plt.title('ch6')
                plt.plot(forcesig1[:, ch], '--')
                plt.plot(self.forceflag_sw * (forcesig1[:, ch].max() - forcesig1[:, ch].min()))
                plt.legend(('forcesig_diff', 'forceflag'))
                if forceflag_true is not None:
                    assert (forceflag_true.shape == self.forceflag_sw.shape)
                    plt.plot(forceflag_true * (forcesig1[:, ch].max() - forcesig1[:, ch].min()) * 1.1)
                    plt.legend(('forcesig_diff', 'forceflag', 'forceflag_true'))
            if (baseline_fw is None) and (stable_state_fw is None):
                plt.show()  # 两个图画完之后才show
        # 图3. 画图比较fw和sw的baseline和stable_state
        if (baseline_fw is not None) or (stable_state_fw is not None) or (forceflag_fw is not None):
            plt.figure()
            ...
            for ch in range(CH_NUMS):
                ...
                if baseline_fw is not None:
                    assert (baseline_fw.shape == self.baseline_sw.shape)
                    plt.plot(self.baseline_sw[:, ch])
                    plt.plot(baseline_fw[:, ch])
                    legend += ('baseline', 'baseline_fw')
                if stable_state_fw is not None:
                    assert (stable_state_fw.shape == self.stable_state_sw.shape)
                    plt.plot(
                        self.stable_state_sw[:, ch] * (
                                self.rawdata[:, ch].max() - self.rawdata[:, ch].min()) + self.rawdata[:,
                                                                                         ch].min())
                    plt.plot((stable_state_fw[:, ch] > 0) * (
                            self.rawdata[:, ch].max() - self.rawdata[:, ch].min()) * 1.1 + self.rawdata[:, ch].min())
                    legend += ('stable_state', 'stable_state_fw')
                if forceflag_fw is not None:
                    assert (forceflag_fw.shape == self.forceflag_sw.shape)
                    plt.plot(
                        self.forceflag_sw * (self.rawdata[:, ch].max() - self.rawdata[:, ch].min()) + self.rawdata[:,
                                                                                                      ch].min())
                    plt.plot(
                        (forceflag_fw > 0) * (
                                self.rawdata[:, ch].max() - self.rawdata[:, ch].min()) * 1.1 + self.rawdata[
                                                                                               :,
                                                                                               ch].min())
